...
function setPseudo() {
  let pseudoLibres = pseudo_Joueur.filter(pseudo => {
    return !players.some(player => player.pseudo === pseudo);
  });

  if (pseudoLibres.length > 0) {
    let index = getRandomIntInclusive(0, pseudoLibres.length - 1);
    prenomJoueur = pseudoLibres[index];
    photoJoueur = image_Joueur[pseudo_Joueur.indexOf(prenomJoueur)];
  } else {}
}
